...
BUILD_COMMAND = "mvnd clean package" # 如果没装 mvnd，改回 mvn clean package
LOG_HEIGHT = 12
# ==========================================
...
def get_pom_version():
    ...
    try:
        ...
        console.print("[bold red]❌ 错误: pom.xml 中无 version[/]")
        sys.exit(1)
...
def git_operations(version):
    ...
    console.print(Panel.fit(
        ...
        f"下一步: 请前往 GitHub Releases 页面基于此 Tag 发布 Release。手动发布后Zenodo会自动同步、生成DOI",
        title="完成",
        border_style="green"
    ))

if __name__ == "__main__":
    console.print(Panel.fit("[bold white]FIA 自动化发布工具[/] [dim](v3.1 - Windows Fix)[/]", style="bold blue"))
    ver = get_pom_version()
